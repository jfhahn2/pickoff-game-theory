...
%%%% Main text entry area:
 ...
 \section{Introduction}
  ...
  \label{sec:sg-model}
    ...
    \noindent This result is proved in the Supplementary Material. Furthermore, the proof of Theorem 1 also establishes a convergence rate of the form $\abs{V^k(s) - V^*(s)} \leq O(\rho^k)$ for all $s \in {\cal S}$ and a $\rho \in (0,1)$.
  ...
  \label{sec:results}
    ...
    \label{sec:results-one-player}
      ...
      \begin{table}
        ...
        \caption{
          \revision{
            \it The increase in optimal lead distance (in inches) after an unsuccessful pickoff attempt under the one-player MDP, assuming an average runner on first base facing an average battery (pitcher and catcher) with third base empty. Estimates include standard errors estimated via bootstrapping.
          }
        }
